...
function builderHtml(project){
  const profile=findDeveloperResearch(project.builder);
  const name=escapeHtml(project.builder);
  return profile?`<a class="developer-detail-link" href="./developers/?builder=${encodeURIComponent(profile.id)}" title="查看 ${escapeHtml(profile.name)} 的建商研究">${name}</a>`:name;
}
function priceHtml(project){
  if(!project.priceEstimate)return escapeHtml(project.price);
  const estimate=project.priceEstimate;
  return `<details class="price-estimate"><summary>${escapeHtml(project.price)}<span>估價依據</span></summary><p>依本站已收錄、可辨識單價的 ${escapeHtml(estimate.basis)} ${estimate.count} 筆案件，以每坪均價的第 25–75 百分位推得；中位數約 ${estimate.median} 萬／坪。未納入樓層、朝向、坪數、車位、個案規格或建商評等加成，不是開價、成交價或估價報告。</p></details>`;
}
